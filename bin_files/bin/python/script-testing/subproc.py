	...
	def main(self, args, unks):
		...
		if args.as_list:
			...
			for (idx,unk) in enumerate(unks):
				if ' ' not in unk:
					quoted_unks.append(unk)
				elif '"' in unk:
					if "'" in unk:
						raise ValueError("Unable to process argument {0}:  [{1}]".format(idx, unk))
					quoted_unks.append("'" + unk + "'")
				else:
					quoted_unks.append('"' + unk + '"')
			cmd = ' '.join(quoted_unks)

		if args.raw_call:
			subprocess.check_call(cmd, shell=not args.no_shell)
		else:
			subproc.runBlockCmd(cmd, logname='log-child.log', shell=not args.no_shell)
		return 0
...
def main():
	...
	ap.add_argument("--no_shell", dest="no_shell", action="store_true", help="launch child python with '-u' switch (for unbuffered i/o)")
	ap.add_argument("--as_list", dest="as_list", action="store_true", help="use subprocess.check_call() instead of custom wrapper")
	args,unks = ap.parse_known_args()
	app = SubProc()
	return app.main(args, unks)
